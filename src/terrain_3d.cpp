...
void Terrain3D::__physics_process(const double p_delta) {
	...
	if (_clipmap_last_position.distance_to(cam_pos_2d) > 0.2f) {
		if (_mesher) {
			_mesher->snap(cam_pos);
		}
		_snapped_position = (cam_pos / _vertex_spacing).floor() * _vertex_spacing;
		_clipmap_last_position = cam_pos_2d;
	}
...
void Terrain3D::set_collision_target_override(Node3D *p_node) {
	LOG(INFO, "Setting collision target override ", p_node);
...
void Terrain3D::set_clipmap_target_override(Node3D *p_node) {
	LOG(INFO, "Setting clipmap target override ", p_node);
