...
void Terrain3DZoneLoader::initialize(Terrain3D *p_terrain) {
	if (p_terrain->has_node(NodePath("ZoneLoader"))) {
		return;
	}
	_terrain = p_terrain;
	set_name("ZoneLoader");
...
void Terrain3DZoneLoader::load() {
	...
	Ref<PackedScene> packed_scene = _region->get_zone_scene();
...
void Terrain3DZoneLoader::save() {
	...
	if (!_data_loaded) {
		LOG(MESG, get_name(), " has no data to save")
	}
	if (!_region.is_valid()) {
		LOG(ERROR, get_name(), " has invalid region pointer")
		return;
	...
	_region->set_zone_scene(packed_scene);
}
...
void Terrain3DZoneLoader::set_region(Ref<Terrain3DRegion> p_region) {
	_region = p_region;
	set_name(Util::location_to_filename(_region->get_location()));
}
...
void Terrain3DZoneLoader::_bind_methods() {
	//ClassDB::bind_method(D_METHOD("load"), &Terrain3DZoneLoader::load);
	//ClassDB::bind_method(D_METHOD("save"), &Terrain3DZoneLoader::save);
	//ClassDB::bind_method(D_METHOD("clear"), &Terrain3DZoneLoader::clear);
	...
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "ZoneLoader/Load", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_EDITOR), "set_load_button", "get_load_button");
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "ZoneLoader/Save", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_EDITOR), "set_save_button", "get_save_button");
	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "ZoneLoader/Clear", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_EDITOR), "set_clear_button", "get_clear_button");
}