...
func _on_setting_changed(p_setting: Variant = null) -> void:
	...
	brush_data["asset_id"] = plugin.asset_dock.get_current_list().get_selected_ids().keys()[0]
	
	# Set asset_ids: Array for instancer
	brush_data["asset_ids"] = plugin.asset_dock.get_current_list().get_selected_ids()
